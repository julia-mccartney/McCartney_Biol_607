...
This Midterm's file can also be found on [Github](https://github.com/julia-mccartney/McCartney_Biol_607/blob/master/exam/McCartney_Midterm.Rmd)
...
# Test function
cases_thru_time(data = mass_cases, cases_col = cases_to_date, mycolor = "blue", y_axis = "Total Cases")
...
```{r 5a LS, warning=FALSE, message = FALSE}
# LS model
...
# Credible intervals - generally pretty tight intervals
...
The confidence intervals varied for all three models, but again were very similar to each other. The Bayesian model had the largest interval for the intercept and slope, followed by the MLE, then LS models - though it is worth noting that the intervals for both estimates across all the models overlapped heavily. 
...
Once we get into the confidence/credible intervals, however, the interpretations change. Whereas the frequentist statistics are looking at a window of values where, with repeated sampling, we would find the parameter value within 95% of the time, the Bayesian approach focuses on the degree of belief in a specific value. Bayesian credible intervals indicate the window in which 95% of all probable values (having a probability density greater than 0) are found. So when we look at the CI values in the chart, the LS and MLE models tell us that if we were to sample quail populations many times, we would see the true slope or intercept value captured within the range of 0.359 and 0.385 95% of the time. Our Bayesian model tells us that, given any estimate of the slope, we can assign a degree of belief in it, from 0 to 1 (theoretically, though with the probability spread out, the maximum value will likely be less than 0.5), and 95% of all our probability (total of 1 or 100%) is found within 0.360 and 0.385. In plain English, the most believable (or credible!) estimate values fall within the credible interval. 
...
We can see that at low sample sizes, the posterior is closely aligned with (heavily influenced by) the prior. Even at 100 samples, there is some overlap in probability of the slope estimate with the prior. However, at sample sizes of 300 and 500, there is no overlap, meaning that the data has overwhelmed the prior completely. For this specific prior, it starts to be mostly overwhelmed at around 100 samples, but is likely not completely overwhelmed until 150-200 samples. This is reflected in the fixed effects, where the 10 samples model maintains the prior mean of 0.7 is within the 95%  credible interval, but this is not the case for the 100, 300, and 500 samples models. 
...
```{r 6 kfold cv set up}
set.seed(739187401)
quail_models <- rsample::vfold_cv(quail, v = 30) %>% 
...
As we can see from the graph, there is not one model that seems to clearly be preforming better than any other (though the 1st and 2nd order models seem to have a higher RMSE). To get a better sense of the data, I calculated the average RMSE for each model.
...
The cubic model has the lowest RMSE, but all of the RMSEs for every model is within the SD range of every other model, so there is not one clear "best" model here. 
  
Ultimately, the models are all extremely similar and seem to preform within a reasonable margin of each other. However, a higher order model (3rd) may describe the data slightly better than lower orders (1st and 2nd). 
